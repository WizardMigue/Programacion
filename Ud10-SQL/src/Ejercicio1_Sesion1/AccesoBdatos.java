...
// SOLO PARA PRUEBAS!!!!!!!
	...
	/*
	 * public Socio consultarId(int socioID){ try { Statement consulta =
	 * conecta.createStatement(); ResultSet reg = consulta.executeQuery
	 * ("SELECT * FROM socio WHERE socioID =" + socioID);
	 * //NOOOOOOOOOOOOOOOOOOOOOOOOOO HACER ESTO NUNCA // SOLO EN ESTE MOMENTO //
	 * UTILIZAREMOS SENTENCIAS PREPARADAS!!!!!!!!!!!!!!!!! Socio socio = new
	 * Socio(); if (reg.next ()) { socio.setSocioID(reg.getInt(1));
	 * socio.setNombre(reg.getString(2)); socio.setEstatura(reg.getInt(3));
	 * socio.setEdad(reg.getInt(4)); socio.setLocalidad(reg.getString(5));
	 * 
	 * 
	 * 
	 * } else socio=null; consulta.close (); return socio; } catch (SQLException e)
	 * { System.out.println("error en la consulta"+e.getMessage()); return null; } }
	 * //de consultarId
	 */
